...
conn = psycopg2.connect(database="covid-nineteen", user="postgres", password="admin", host="localhost", port="5432")
...
for row in rows:
    ...
    if row[7] != None:
        timeout_status = 'On Time' if row[7] >= dailytimeout else 'Early'
    ...
    if reachable == 0:
        ...
        cur.execute(f"UPDATE user_user set timeout = '{current_time}', timeout_status = '{timeout_status}', status = 'Offline', present = 'False' where user_idnumber = '{row[0]}'")
        conn.commit()
